...
def main():
    ...
    # for di in datapath:
    #     print(di)
    args.save_dir = os.path.join(args.save_dir, datapath[len(datapath)-1])
    print(args.save_dir)
    ...
    for iter_i in range(start_iter, max_iter + 1):
        ...
        if iter_i % args.save_interval == 0 or iter_i == max_iter:
            ...
            if iter_i == max_iter:
                ...
                state_dict_path = os.path.join(args.save_dir, f"{model_name}_final.pth")
                # state_dict_path = args.save_dir / f"{model_name}_final.pth"
                history_path = os.path.join(args.save_dir, "history_final.csv")
                ...
                state_dict_path = os.path.join(args.save_dir, f"{model_name}_{iter_i:06d}.ckpt")
                history_path = os.path.join(args.save_dir, f"history_{iter_i:06d}.csv")
